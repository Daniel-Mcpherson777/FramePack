"""
Clean FramePack inference without Gradio dependencies
Extracted from demo_gradio.py for RunPod serverless usage
"""
...
# H100 80GB needs VAE slicing to avoid OOM - keep threshold high to force low-VRAM mode
...
# Move models to GPU or enable dynamic swapping
if not high_vram:
...
def generate_video(input_image, prompt, n_prompt="", seed=12345, total_second_length=5,
    ...
    print(f"Starting video generation - Job ID: {job_id}")
    ...
    try:
        ...
        for latent_padding in latent_paddings:
            ...
            def callback(d):
                current_step = d['i'] + 1
                print(f'Sampling {current_step}/{steps} - Total frames: {int(max(0, total_generated_latent_frames * 4 - 3))}')
                return
